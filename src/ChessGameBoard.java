...
public class ChessGameBoard extends JPanel{
    ...
    public void initializeBoard() {
        ...
        for (int i = 0; i < chessCells.length; i++) {
            for (int j = 0; j < chessCells[0].length; j++) {
                ...
                setBoardSquare(i, j, pieceToAdd);
                setBoardSquareBackground(i, j);
                addMouseListenerToBoardSquare(i, j);
                addToBoard(chessCells[i][j]);
            }
        }
    }
    ...
    private void setBoardSquare(int i, int j, ChessGamePiece pieceToAdd) {
        chessCells[i][j] = new BoardSquare(i, j, pieceToAdd);
    }

    private void setBoardSquareBackground(int i, int j) {
        if ((i + j) % 2 == 0) {
            chessCells[i][j].setBackground(Color.WHITE);
        } else {
            chessCells[i][j].setBackground(Color.BLACK);
    ...
    private void addMouseListenerToBoardSquare(int i, int j) {
        chessCells[i][j].addMouseListener(listener);
    }

    private void addToBoard(BoardSquare boardSquare) {
